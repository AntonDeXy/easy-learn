...
const AddWord:React.FC<AddWordType> = ({autoTranslates, translatesLanguage, closeModal, currentListId, getAutoTranslatesThunk, createItemThunk, disabledButtonStyle}) => {
  ...
  const getAutoTranslates = async (phrase:any) => {
    if (phrase.length > 0) {
      setTranslatesLoading(true)
      await getAutoTranslatesThunk(
        phrase,
        translatesLanguage
      )
            ...
            <ul>
              {
                autoTranslates?.length > 0 && autoTranslates.map(item => (
                  <li onClick={() => chooseTranslate(item)} >{item}</li>
                ))
              }
            </ul>
          </div>
